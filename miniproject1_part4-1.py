...
def get_sorted_cosine_similarity(text_search, embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        for category in categories:
            ...
            sim = cosine_similarity(input_embedding, cat_embedding)
        ...
        for category in categories:
            # Check if the category embedding is already computed
    ...
    sorted_cosine_sim = sorted(((category_indices[category], sim) for category, sim in cosine_sim.items()), key=lambda kv: kv[1], reverse=True)
